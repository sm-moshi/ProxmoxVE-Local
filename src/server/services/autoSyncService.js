...
export class AutoSyncService {
  ...
  safeToISOString(date) {
    try {
      ...
      if (!date || isNaN(date.getTime())) {
        console.warn(
          "Invalid date provided to safeToISOString, using current time as fallback",
        );
  ...
  scheduleAutoSync() {
    ...
    if (globalAutoSyncLock) {
      console.log(
        "Auto-sync is already running globally, not scheduling new cron job",
      );
      return;
    }
    ...
    if (settings.syncIntervalType === "custom") {
      cronExpression = settings.syncIntervalCron || cronExpression;
    } else {
      ...
      cronExpression =
        intervalMap[settings.syncIntervalPredefined ?? "1hour"] ?? "0 * * * *";
    }
    ...
    this.cronJob = cron.schedule(
      ...
      async () => {
        ...
        if (globalAutoSyncLock) {
          console.log(
            "Auto-sync already running globally, skipping cron execution...",
          );
          return;
        }
        ...
        if (!currentSettings.autoSyncEnabled) {
          console.log(
            "Auto-sync has been disabled, stopping and destroying cron job",
          );
          this.stopAutoSync();
          return;
        }
    ...
    try {
      ...
      try {
        ...
        const logoEntries = pbScripts
          ...
          .map((pb) => ({
            slug: pb.slug,
            url: /** @type {string} */ (pb.logo),
          }));
        const logoResult = await cacheLogos(logoEntries);
        console.log(
          `Logo cache: ${logoResult.downloaded} new, ${logoResult.skipped} cached, ${logoResult.errors} errors`,
        );
      } catch (logoErr) {
      ...
      const { scriptDownloaderService: sds } =
        await import("./scriptDownloader.js");
      ...
      if (settings.autoDownloadNew || settings.autoUpdateExisting) {
        ...
        for (const script of allScripts) {
          try {
            ...
            const isDownloaded =
              await scriptDownloaderService.isScriptDownloaded(script);
            if (!isDownloaded) {
            ...
            console.warn(
              `Error checking script ${script?.slug || "unknown"}:`,
              error,
            );
        ...
        console.log(
          `Found ${newScripts.length} new scripts and ${existingScripts.length} existing scripts`,
        );
        ...
        if (settings.autoUpdateExisting && existingScripts.length > 0) {
          console.log(
            `Auto-updating ${existingScripts.length} existing scripts...`,
          );
          const updated = [];
          const errors = [];
        ...
        console.log(
          "Auto-download/update disabled, skipping script processing",
        );
      }
  ...
  groupScriptsByCategory(scripts) {
    ...
    scripts.forEach((script) => {
      if (!script || !script.name) {
        console.warn(
          "Invalid script object in groupScriptsByCategory, skipping:",
          script,
        );
        return;
      }
      ...
      const categoryNames =
        Array.isArray(script.categories) && script.categories.length > 0
          ? script.categories.map((/** @type {any} */ c) =>
              typeof c === "string" ? c : (c?.name ?? "Miscellaneous"),
            )
          : ["Miscellaneous"];
   ...
   * @param {AutoSyncResults} results - Sync results object
   */
  async sendSyncNotification(results) {
    ...
    if (results.updatedScripts?.length > 0) {
      ...
      // @ts-ignore - Dynamic property access
      const updatedScriptsGrouped = this.groupScriptsByCategory(
        results.updatedScripts,
      );
    ...
    // @ts-ignore - Dynamic property access
    if (
      results.newScripts?.length === 0 &&
      results.updatedScripts?.length === 0 &&
