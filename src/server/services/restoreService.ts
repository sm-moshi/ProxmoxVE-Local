...
class RestoreService {
  ...
  async getRootfsStorage(server: Server, ctId: string): Promise<string | null> {
    ...
    try {
      ...
      const script = installedScripts.find(
        (s: any) => s.container_id === ctId && s.server_id === server.id,
      );
        ...
        (code: number) => {
          ...
          if (exitCode === 0) {
            ...
            if (
              ...
              reject(
                new Error(
                  `Destroy failed with exit code ${exitCode}: ${output}`,
                ),
              );
            }
          }
      ...
      sshService.executeCommand(
        ...
        (code: number) => {
          ...
          if (exitCode === 0) {
            ...
            reject(
              new Error(`Restore failed with exit code ${exitCode}: ${output}`),
            );
          }
        },
      );
  ...
  async restorePBSBackup(
    ...
    if (!pbsIp || !pbsDatastore) {
      throw new Error(
        `Missing PBS IP or datastore for storage ${storage.name}`,
      );
    }
    ...
    if (onProgress)
      await onProgress("pbs_download", "Downloading backup from PBS...");
          ...
          sshService.executeCommand(
            ...
            (code: number) => {
              ...
              if (exitCode === 0) {
                ...
                reject(
                  new Error(
                    `Download failed with exit code ${exitCode}: ${output}`,
                  ),
                );
              }
            },
          ...
          sshService.executeCommand(
            ...
            (code: number) => {
              ...
              if (packExitCode === 0) {
                ...
                reject(
                  new Error(
                    `Pack failed with exit code ${packExitCode}: ${packOutput}`,
                  ),
                );
              }
            },
    ...
    try {
      ...
      if (onProgress)
        await onProgress("cleanup", "Cleaning up temporary files...");
    ...
    try {
      ...
      if (!rootfsStorage) {
        ...
        if (checkOutput.includes("notfound")) {
          ...
          throw new Error(
            `Container ${containerId} does not exist and storage could not be determined. Please ensure the container exists or specify the storage manually.`,
          );
        }

        throw new Error(
          `Could not determine rootfs storage for container ${containerId}. Please ensure the container exists and has a valid configuration.`,
        );
      }
      ...
      try {
        ...
        await addProgress(
          "skipping",
          "Container does not exist or already destroyed, continuing...",
        );
      }
      ...
      if (backup.storage_type === "pbs") {
        ...
        const snapshotPathMatch =
          backup.backup_path.match(/pbs:\/\/[^/]+\/(.+)$/);
        if (!snapshotPathMatch || !snapshotPathMatch[1]) {
          throw new Error(
            `Invalid PBS backup path format: ${backup.backup_path}`,
          );
        }

        const snapshotPath = snapshotPathMatch[1];

        await this.restorePBSBackup(
          server,
          storage,
          containerId,
          snapshotPath,
