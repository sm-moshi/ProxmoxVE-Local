...
export const versionRouter = createTRPCRouter({
    ...
    try {
      ...
      if (allowPrerelease) {
        const response = await fetchGitHubAPI(
          "https://api.github.com/repos/community-scripts/ProxmoxVE-Local/releases",
        );
        if (!response.ok)
        ...
        const response = await fetchGitHubAPI(
          "https://api.github.com/repos/community-scripts/ProxmoxVE-Local/releases/latest",
        );
        if (!response.ok)
          throw new Error(`GitHub API error: ${response.status}`);
  ...
  getVersionStatus: publicProcedure.query(async () => {
    try {
      const versionPath = join(process.cwd(), "VERSION");
  ...
  getAllReleases: publicProcedure.query(async () => {
    try {
      const response = await fetchGitHubAPI(
        "https://api.github.com/repos/community-scripts/ProxmoxVE-Local/releases",
      );
  ...
  getUpdateLogs: publicProcedure.query(async () => {
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to read update logs",
        logs: [],
        isComplete: false,
        logFileModifiedTime: null,
      };
  ...
  executeUpdate: publicProcedure.mutation(async () => {
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          await writeFile(
            logPath,
            "[INFO] Fetched latest update.sh from GitHub\n",
            { flag: "a" },
          );
        } else {
          ...
          await writeFile(
            logPath,
            `[WARNING] Could not fetch latest update.sh (HTTP ${response.status}), using local version\n`,
            { flag: "a" },
          );
        }
      } catch (fetchError) {
        ...
        await writeFile(
          logPath,
          `[WARNING] Could not fetch latest update.sh: ${errorMsg}, using local version\n`,
          { flag: "a" },
        );
      }
      ...
      return {
        success: true,
        message:
          "Update started in background. The server will restart automatically when complete.",
        output: "",
        error: "",
      };
    } catch (error) {
