...
import {
  cacheLogos,
  getLocalLogoPath,
} from "~/server/services/logoCacheService";
...
function pbToScript(pb: PBScript): Script {
  return {
    ...
    install_methods: pb.install_methods.map((m) => ({
      ...
      script:
        scriptDownloaderService.deriveScriptPath(pb.type, m.type, pb.slug) ??
        undefined,
    })),
    default_credentials: {
...
export const scriptsRouter = createTRPCRouter({
  ...
  getCtScripts: publicProcedure.query(async () => {
  ...
  getAllDownloadedScripts: publicProcedure.query(async () => {
  ...
  getScriptCardsWithCategories: publicProcedure.query(async () => {
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch script cards with categories",
        cards: [],
        metadata: null,
      };
    }
  ...
  resyncScripts: publicProcedure.mutation(async () => {
    try {
  ...
  loadScript: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (
          UNSUPPORTED_TYPES.includes(
            pb.type as (typeof UNSUPPORTED_TYPES)[number],
          )
        ) {
          return {
            success: false,
            error: `Script type '${pb.type}' is not supported yet`,
            files: [],
          };
        }
      ...
      try {
          ...
          const results = await Promise.allSettled(
            batch.map(async (slug) => {
              ...
              if (
                ...
                throw Object.assign(
                  new Error(`Script type '${pb.type}' is not supported yet`),
                  { slug },
                );
              }
              const result = await scriptDownloaderService.loadScript(
                pbToScript(pb),
              );
              if (!result.success) {
                const error =
        ...
        return {
          ...
          error:
            error instanceof Error
              ? error.message
              : "Failed to load multiple scripts",
          successful: [],
          failed: [],
          total: 0,
        };
  ...
  checkScriptFiles: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const result = await scriptDownloaderService.checkScriptExists(
          pbToScript(pb),
        );
  ...
  deleteScript: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const result = await scriptDownloaderService.deleteScript(
          pbToScript(pb),
        );
        return result;
      } catch (error) {
  ...
  compareScriptContent: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const result = await scriptDownloaderService.compareScriptContent(
          pbToScript(pb),
        );
        return { success: true, ...result };
      } catch (error) {
        ...
        return {
          ...
          error:
            error instanceof Error
              ? error.message
              : "Failed to compare script content",
          hasDifferences: false,
          differences: [],
        };
      }
  ...
  getScriptDiff: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const result = await scriptDownloaderService.getScriptDiff(
          pbToScript(pb),
          input.filePath,
        );
        return { success: true, ...result };
  ...
  checkProxmoxVE: publicProcedure.query(async () => {
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to check Proxmox VE status",
        message: "Failed to check Proxmox VE status",
      };
    }
  }),
  ...
  getAutoSyncSettings: publicProcedure.query(async () => {
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to get auto-sync settings",
        settings: null,
      };
    }
  }),
      ...
      try {
        ...
        return {
          ...
          error:
            error instanceof Error
              ? error.message
              : "Failed to save auto-sync settings",
        };
      }
    }),
  ...
  triggerManualAutoSync: publicProcedure.mutation(async () => {
    try {
      const autoSyncService = new AutoSyncService();
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to execute manual auto-sync",
        result: null,
      };
    }
  }),

  getAutoSyncStatus: publicProcedure.query(async () => {
    try {
      const autoSyncService = new AutoSyncService();
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to get auto-sync status",
        status: null,
      };
    }
  }),
