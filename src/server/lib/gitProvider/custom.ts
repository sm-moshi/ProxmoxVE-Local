...
export class CustomProvider implements GitProvider {
  async listDirectory(
    ...
    if (!response.ok) {
      throw new Error(
        `Custom Git server: list directory failed (${response.status}).`,
      );
    }
    ...
    if (!Array.isArray(data)) {
      const single = data as unknown as {
        type?: string;
        name?: string;
        path?: string;
      };
  ...
  async downloadRawFile(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to download ${filePath} from custom Git server (${response.status}).`,
      );
    }
