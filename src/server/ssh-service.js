  ...
  async testConnection(server) {
    ...
    return new Promise((resolve) => {
      ...
      if (auth_type === "key") {
        ...
        authPromise = this.testWithSshpass(server).catch(() =>
          this.testWithExpect(server),
        );
      }
      ...
      setTimeout(() => {
        if (!resolved) {
          ...
          resolve({
            success: false,
            message:
              "SSH login test timeout - server did not respond within 15 seconds",
        ...
        if (!resolved) {
          ...
          if (code === 0 && output.includes("SSH_LOGIN_SUCCESS")) {
            ...
            if (
              ...
              errorMessage =
                "Connection refused - server may be down or SSH not running";
            } else if (
    ...
    return new Promise((resolve) => {
        ...
        if (!resolved) {
          ...
          if (code === 0) {
            ...
            resolve({
              success: false,
              message:
                "SSH port (22) is not accessible - server may be down or SSH not running",
              details: {
                portOpen: false,
                exitCode: code,
                method: "port_check",
      ...
      portTestCommand.on("error", (error) => {
        if (!resolved) {
          ...
          this.testWithTelnet(server)
            .then(resolve)
            .catch(() => {
              resolve({
                success: false,
                message:
                  "Cannot test SSH connectivity - netcat and telnet not available",
                details: {
                  error: error.message,
      ...
      telnetCommand.on("error", (error) => {
        if (!resolved) {
          ...
          resolve({
            success: false,
            message:
              "Cannot test SSH connectivity - required tools not available",
            details: {
              error: error.message,
              method: "telnet_error",
            },
        ...
        if (!resolved) {
          ...
          if (
            ...
            resolve({
              success: true,
              message:
                "SSH service is running and accessible - authentication required",
              details: {
                server: server.name || "Unknown",
                ip: ip,
                user: user,
          ...
          if (!resolved) {
            ...
            if (code === 0 && output.includes("SSH_LOGIN_SUCCESS")) {
              resolve({
                success: true,
                message:
                  "SSH key authentication successful - credentials verified",
                details: {
                  server: server.name || "Unknown",
                  ip: ip,
                  user: user,
              ...
              if (
                ...
                errorMessage =
                  "SSH key authentication failed - check key and permissions";
              } else if (errorOutput.includes("Connection refused")) {
                errorMessage =
                  "Connection refused - server may be down or SSH not running";
              } else if (
                ...
                errorOutput.includes("Load key") ||
                errorOutput.includes("invalid format")
              ) {
      ...
      sshKeygen.on("close", (code) => {
        if (code === 0) {
          try {
            ...
            reject(
              new Error(
                `Failed to read generated key files: ${error instanceof Error ? error.message : String(error)}`,
              ),
            );
          }
        } else {
